...
## Create a Free Trial of Microsoft Copilot Studio

From within the lab virtual machine, launch edge and navigate to the following URL:

```
https://go.microsoft.com/fwlink/p/?linkid=2252605
```

Use the login and password located in the Environment tab of the lab player to start the free trial. 
...
1. Navigate to **Copilot Studio** at the below url, signing in with the **Micrsoft Cloud** credentials located in the **Environment** tab of the lab player. Skip any welcome messages.
...
1. Select the **+ Add a topic** menu, then select **Add from description with Copilot**.
...
1. Open a new browser tab and download the `expenses policy document` file from the below URL, saving it locally. This document contains details of the expenses policy for the fictional Contoso corporation.
...
1. In the **Upload file** section, upload the expense policy document you downloaded previously and add it to your agent's knowledge.
    ...
    > **Note**: After uploading the file, you will need to wait while it is indexed; which may take 10 minutes (or longer). Now might be a good time to take a coffee break while you review the `expenses policy document`!
...
> **Tip**: If you need help, consult the `Copilot Studio documentation` at the below url.
